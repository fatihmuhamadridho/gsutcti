...
export const getPageSearch = ({ query, page, per_page }: any) => async(dispatch: any) => {
    try {
        console.log("params", query)
        dispatch({ type: SEARCH_ACTION })
        const res = await axios.get(`${baseUrl}q=${String(query)}&page=${page ? String(page) : "1"}&per_page=${per_page ? String(per_page) : "5"}`)
        dispatch({
